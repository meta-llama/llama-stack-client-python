...
class Agent:
    ...
    def _run_tool(self, tool_calls: List[ToolCall]) -> List[ToolResponseParam]:
        responses = []
        ...
        while not is_turn_complete:
            ...
            for chunk in turn_response:
                ...
                if not tool_calls:
                    ...
                    # run the tools
                    tool_responses = self._run_tool(tool_calls)
...
class AsyncAgent:
    ...
    async def _run_tool(self, tool_calls: List[ToolCall]) -> List[ToolResponseMessage]:
        responses = []
        for tool_call in tool_calls:
            responses.append(await self._run_single_tool(tool_call))
        return responses

    async def _run_single_tool(self, tool_call: ToolCall) -> ToolResponseMessage:
        # custom client tools
        if tool_call.tool_name in self.client_tools:
            tool = self.client_tools[tool_call.tool_name]
        ...
        while not is_turn_complete:
            ...
            async for chunk in turn_response:
                ...
                if not tool_calls:
                    ...
                    # run the tools
                    tool_response_messages = await self._run_tool(tool_calls)
